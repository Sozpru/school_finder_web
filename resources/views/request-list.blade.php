...
<html lang="en">
    ...
    <body class="bg-blue">
                    ...
                    <div class="col-lg-6 col-md-6">
                        ...
                        <div class="count-box tutor-img">
                            ...
                            <div>
                                ...
                                <div class="position-absolute bottom-0 end-0 mr-4">
                                    @if($request->status == '')  
                                    <a class="btn btn-outline-success m-2 opacity-75" id="{{ $request->id }}" value="accept"  onclick="update_status(this.id)">Accept</a>
                                    <a class="btn btn-outline-danger opacity-75" id="{{ $request->id }}" value="delete"  onclick="delete_status(this.id)">Delete</a>
                                    @elseif($request->status == 'accept')
                                    <a class="btn btn-outline-success m-2 opacity-75"  data-email="{{ $request->student_email }}" data-mobile="{{ $request->mobile_no }}" data-subject="{{ $request->sname }}" data-standard="{{ $request->standard }}" data-tuition-type="{{ $request->tuition_type }}"  data-pstatus="{{ $request->pstatus }}" data-location="{{ $request->location }}" data-m_hours="{{ $request->m_hours }}" data-a_hours="{{ $request->a_hours }}" data-amount="{{ $request->amount }}" data-transaction_id="{{ $request->transaction_id }}" data-datetime="{{ $request->datetime }}" onclick="get_student_details(this)">View Details</a>
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="modal fade" id="StudentDetailModal1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content top-50 shadow border-0">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-success m-2 opacity-75" onclick="update_request();">Accept</button>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <center>
                        <div class="modal-body mb-1">
                            <div class="tutor-img" id="get_tutor_data">
                                <img src="{{ (isset($request)) ? ((empty($request->image)) ? asset('assets/img/avatar.png') : asset($request->image) ) : asset('assets/img/avatar.png') }}" class="rounded">
                            </div>
                            ...
                            <div class="student-detail text-left">
                                ...
                                <p class="font-weight-light mb-1">Tuition Type</p>
                                <h5 id="tuition"></h5>
                                <hr/>
...
<!-- <a href="#" class="back-to-top d-flex align-items-center justify-content-center"><i class="bi bi-arrow-up-short"></i></a> -->
    ...
    <script type="text/javascript">
                                        ...
                                        function get_student_details(val) {
                                            ...
                                            var status = $(val).attr("data-pstatus");
    ...
    <script>
        ...
        function update_status(id) {
            ...
            $.ajax({
                url: "<?php echo URL('request_tutor_details') ?>",
                type: "post",
                data: dataString,
                cache: false,
                success: function (data) {
                    //alert(data);
                    $('#StudentDetailModal1').modal('show');
                    document.getElementById('get_tutor_data').innerHTML = data;
                }
            });
        }
        function delete_status(id) {
            var del = confirm("Are you sure you want to delete this Request?");
            if (del == true) {
                var dataString = 'id=' + id;
                $.ajax({
                    url: "<?php echo URL('delete_request') ?>",
                    type: "post",
                    data: dataString,
                    cache: false,
                    success: function (data) {
        ...
        function update_request() {
            ...
            var dataString = 'id=' + id;
            $.ajax({
                url: "<?php echo URL('update_request') ?>",
                type: "post",
                data: dataString,
                cache: false,
                success: function (data) {
